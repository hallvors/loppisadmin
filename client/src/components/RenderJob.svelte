<script>
  ...
  function update(event) {
    showEditor = false;
    return changeJobDetails(job[cols.JOBNR], cols, event.detail).catch((err) =>
      alert(err),
    );
  }
  ...
  function getDriverName(number) {
    let driver = $drivers.find(
      (driver) => driver.number === normalizeNumber(number),
    );
  ...
  function removeAssignee() {
    if (
      confirm(
        "Vil du fjerne henter-informasjon fra denne jobben? Merk at henteren ikke får noen beskjed om dette automatisk.",
      )
    ) {
      return changeJobDetails(job[cols.JOBNR], cols, {
        [cols.STATUS]: "Ny",
        [cols.ASSIGNEE]: "",
      }).catch((err) => alert(err));
    }
...
<tr class="job" class:itemSelected data-id={job[cols.JOBNR]}>
  ...
  >
    ...
    {job[cols.ADDRESS]}
    <a
      href="https://www.google.no/maps/?q={encodeURIComponent(
        job[cols.ADDRESS],
      )}"
      on:click|stopPropagation
      target="_blank">🔎</a
    >
    <br /><small class="floor">{job[cols.FLOOR]}</small>
  ...
  <td
    ...
    on:click={(e) => {
      if (
        ["SELECT", "LABEL", "INPUT", "OPTION", "A"].indexOf(
          e.target.tagName,
        ) === -1
      ) {
  ...
  >
    <input
      type="checkbox"
      bind:checked={itemSelected}
      id="select{job[cols.JOBNR]}"
      on:change={(e) =>
        dispatch("select", {
    ...
    <select
      ...
      on:change|stopPropagation={(e) =>
        update({ detail: { [cols.STATUS]: e.target.value } })}
    ...
    {#if job[cols.ASSIGNEE]}
      <div class="hentesav">
        <a href="tel:{normalizeNumber(job[cols.ASSIGNEE])}">
          {getDriverName(job[cols.ASSIGNEE])}</a
        >
        {statusVerbString(job[cols.STATUS])}
        <button on:click|stopPropagation={(evt) => removeAssignee(evt)}
          >❌</button
        >
      </div>
    {/if}
...
{#if expanded}<tr data-id={job[cols.JOBNR]}
    ><td></td><td colspan="3" class="extrainfo">
      <RenderPerson name={job[cols.CONTACT_PERSON]} number={job[cols.PHONE]} />
      <p>{job[cols.TYPES]}</p>
      <p><i>{job[cols.DESC]}</i></p>
      <p class="cen">
        <button on:click={(e) => (showEditor = true)}
          ><img
            src="/images/edit.png"
            alt="endre detaljer"
            width="36"
    ...
    <td colspan="2">
      ...
      <textarea
        ...
        on:change={(e) =>
          changeJobDetails(job[cols.JOBNR], cols, {
            [cols.ADMCOMMENT]: e.target.value,
          })}
      ></textarea>
    </td>
  </tr>
